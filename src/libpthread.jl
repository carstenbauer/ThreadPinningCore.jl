...
Base.show(io::IO, cpuset::Ccpu_set_t) =
    print(io, "Ccpu_set_t(", reverse(join(bitstring.(reverse(cpuset.bits)))), ")")
function Ccpu_set_t(mask::BitArray)
    n = 64
    maxn = 1024
    length(mask) <= maxn || throw(
        ArgumentError(
            "Input mask (BitArray) is too large (i.e. has more than $maxn elements).",
        ),
    )
